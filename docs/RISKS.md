...
|---|---|---|---|---|
| R-1 | Startup recovery uses the existing FIFO, so a large old backlog can delay a newly finalized live chunk. | Automatic priority scheduling was explicitly classified as nice-to-have; recovery is capped at 32 and all audio remains durable. Revisit if startup recovery causes observed live latency. | Xavier Dedenbach | 2026-09-03 |
...
| Path | Risk IDs | Note |
|---|---|---|
| `scripts/dictation/dictation.py` | R-1, R-2, R-3 | Startup recovery FIFO, recovery-state handling, and bounded shutdown |
